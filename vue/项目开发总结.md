...
**解决方法**：在根组件`app.vue`的`created`和`beforeunload`函数中分别读取`sessionStorage`中的数据和将vuex中store的数据存入`sessionStorage`中。在将sessionStorage中的数据存入store时，可以直接调用vuex.store的[replaceState](https://vuex.vuejs.org/zh/api/#replacestate)方法，替换store的根状态。`replaceState(state:Object)`。
