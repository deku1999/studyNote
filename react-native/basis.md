...
2. 使用谷歌浏览器来调试（在安卓界面ctrl+m然后点击debug即可）
   1. 使用谷歌浏览器即可
   2. 不能查看标签结构
   3. 不能查看网络请求
   
2. 使用rn推荐的工具 [react-native-debugger](https://github.com/jhen0409/react-native-debugger)来调试 (**老师推荐使用这种方式**)（解压之后运行exe之后也是用安卓ctrl+m选debug)
